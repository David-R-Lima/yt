...
import {
  ...
  GetSongsOptions,
  ISongRepository,
} from 'src/domain/repositories/i-song-repository'
...
export class PrismaSongRepository implements ISongRepository {
  ...
  async getFromPlaylist({
    playlistId,
    getSongOptions,
  }: {
    playlistId: string
    getSongOptions: GetSongsOptions
  }): Promise<Song[]> {
    const { startId, excludedIds = [], random = false, reverse } = getSongOptions

    const baseConditions: Prisma.Sql[] = [Prisma.sql`"PlaylistSongs"."playlistId" = ${playlistId}`]
  ...
  async getRecommended(): Promise<Song[]> {
    ...
    const topSongIds = topSongs.map((song) => song.songId)

    const topSongsWithArtists = await this.prisma.songs.findMany({
      where: {
        id: {
          in: topSongIds,
        },
    ...
    const recommendedSongs = await this.prisma.songs.findMany({
      where: {
        artist: {
          in: artists,
        },
